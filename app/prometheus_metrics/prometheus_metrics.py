...
def count_size_ingress():
    if request.content_length:
        counter_ingress.labels(service="post_comment_service").inc( 
...
def count_size_egress(response):
    if response.content_length:
        counter_egress.labels(service="post_comment_service").inc( 
...
def init_metrics():
    ...
    metrics.register_default(
        metrics.counter(
            ...
            labels={
                "service": lambda: "post_comment_service", 
    ...
    def page_not_found(e):
        counter_404.labels(service="post_comment_service", endpoint=request.path).inc() 
        return e, 404
